...
# Read in the dataset
reg_df <- read_csv('data/machine_learning/20240701_defluorinases_entire_alignment_for_classification.csv')
colnames(reg_df)

# Remove variables with nonzero variance (optional)
nozdat <- caret::nearZeroVar(reg_df, saveMetrics = TRUE, uniqueCut = 1)
...
rawdat <- reg_df  %>%
  select(-which_rem, -1)
dat_dedup <- rawdat[!duplicated(rawdat),] 

dat <- bind_cols(nams = reg_df$nams[!duplicated(rawdat)],
                 dat_dedup)
colnames(dat)[ncol(dat)] <- "truth"

# Read in the random forest model from cross-validation
rf_20240701 <- readRDS("data/machine_learning/20240701_classification_random_forest.rds")
...
write_tsv(traindf,"data/machine_learning/20240701_classification_mean_training_accuracy_1000_test_train_splits.txt")
...
write_tsv(tibble(mean_testing_vec),"data/machine_learning/20240701_classification_mean_testing_accuracy_1000_test_train_splits.txt")
mean(mean_training_vec)
sd(mean_training_vec) 
...
write_csv(allsumm, "data/machine_learning/20240701_classification_train_test_1000_summaries.csv")
...
dat_df
write_csv(dat_df, "data/Machine_learning/20240701_full_alignment_classification_accuracy_1000_training_test_combined.csv")
